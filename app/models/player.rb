class Player < ActiveRecord::Base
  ...
  def play(card_id, target_id)
    ...
    if card.category == 'M'
      ...
      game.notifications.create(body: "#{self.user_name} played a #{card.category} with #{card.value} gold")
      ...
      if valid_color?(card, merc)
        ...
        game.notifications.create(body: "#{self.user_name} played a #{card.category} with #{card.value} cannons on #{merc.category} #{merc.value} gold")
        merc.set_leader
        self.cards.delete(card_id)
        hand_card.destroy
        return true
